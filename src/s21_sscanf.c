...
int s21_sscanf(const char *input, const char *format, ...) {
  ...
  while (status != MISMADCH_ && status != ENDET_) {
    ...
    inputParsing(inputStatic, currentInputElem, Specif.width, &inputLoaded, Specif, &startParsing, &n_counter);
  ...
  if ((inputStatic[j] == '+' || inputStatic[j] == '-') && (wid > 1 || wid == -1)) {
    startParse++;
      ...
      if ((*Specif).width == -1) {
        ...
        || (inputStatic[(*Specif).width] == '.' && checkDot == 0 && !checkNan)) {
          if (inputStatic[(*Specif).width] == '.') { // встречаем точку только 1н раз в числе
            checkDot = 1;
          }
          (*Specif).width++;
        }
        if ((*Specif).width == startParse || ((*Specif).width == startParse + 1 && inputStatic[startParse] == '.'&& !checkNan)) {
          *stopMove = true;
        ...
        || (inputStatic[i] == '.' && checkDot == 0 && !checkNan)) && findNotNumber > 0) {
          if (inputStatic[i] == '.') {
            checkDot = 1;
          }
          findNotNumber--;
...
void inputParsing(char inputFlip[16384], char currentInputElem[8192], int wid,
  ...
  fillOneByOne(inputFlip, currentInputElem, wid, Specif, startParsing, n_counter);
